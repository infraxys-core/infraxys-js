(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["InfraxysJs"] = factory();
	else
		root["InfraxysJs"] = factory();
})(self, function() {
...
/* harmony import */ var _base64__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(27);
...
const triggerAction = async (actionAlias, actionArguments = undefined, timeoutSeconds = 600, returnLog = false) => {
  return await _executeAction(actionAlias, actionArguments, true, timeoutSeconds, returnLog);
};

const runAction = async (actionAlias, actionArguments = undefined, timeoutSeconds = 600, returnLog = false) => {
  return await _executeAction(actionAlias, actionArguments, false, timeoutSeconds, returnLog);
};

const _executeAction = async (actionAlias, actionArguments, asynchronous, timeoutSeconds, returnLog) => {
...
const axiosPost = async (url, data = undefined) => {
  const response = await axios__WEBPACK_IMPORTED_MODULE_0___default().post(url, data);
  return response;
};

const uploadFileToProjectCache = async (projectGuid, filePath, contents) => {
  const base64Contents = _base64__WEBPACK_IMPORTED_MODULE_1__.default.encode(contents);
  const url = `${infraxysUrl}api/v1/cache/projects/${projectGuid}/${filePath}`;
  axios__WEBPACK_IMPORTED_MODULE_0___default().post(url, base64Contents).then(response => {
    return 'File pushed';
  });
};
...
const followAction = (workflowRunGuid, actionRunnerGuid) => {
  ...
  evtSource.onmessage = event => {
    var text = _base64__WEBPACK_IMPORTED_MODULE_1__.default.decode(event.data);
    processText(text);
